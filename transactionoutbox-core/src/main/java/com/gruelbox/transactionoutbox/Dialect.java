...
public enum Dialect {
  ...
  POSTGRESQL_9(
      ...
      "DELETE FROM {{table}} WHERE ctid IN (SELECT ctid FROM {{table}} WHERE nextAttemptTime < ? AND processed = true AND blocked = false LIMIT {{batchSize}})"),
  H2(
      Constants.DEFAULT_SELECT_AND_LOCK_ENTRY_WITHOUT_SKIP_LOCKED_STMT,
      Constants.DEFAULT_SELECT_BATCH_ENTRIES_WITHOUT_SKIP_LOCKED_STMT),
  SQL_SERVER_2012(
  ...
  /**
   * @return Format string fo the SQL required to select and lock a record for processing. Should
  ...
  /**
   * @return Format string fo the SQL required to select a batch of entries for processing. Should
  ...
  /** @return Format string fo the SQL required to delete an entry. */
  @SuppressWarnings("JavaDoc")
  ...
  private static class Constants {
    ...
    static final String DEFAULT_SELECT_BATCH_ENTRIES_WITHOUT_SKIP_LOCKED_STMT =
        "SELECT {{fields}} FROM {{table}} WHERE nextAttemptTime < ? AND blocked = false AND processed = false LIMIT {{batchSize}}";
    ...
    static final String DEFAULT_DELETE_EXPIRED_STMT =
        "DELETE FROM {{table}} WHERE nextAttemptTime < ? AND processed = true AND blocked = false LIMIT {{batchSize}}";
